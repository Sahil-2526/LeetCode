class Solution {
public:
    vector<int> seg;
    const int MAX = 50000;
    void update(int node, int l, int r, int idx, int val) {
        if (l == r) {
            seg[node] = val;
            return;
        }
        int mid = (l + r) / 2;
        if (idx <= mid) update(2 * node, l, mid, idx, val);
        else update(2 * node + 1, mid + 1, r, idx, val);
        seg[node] = max(seg[2 * node], seg[2 * node + 1]);
    }
    int query(int node, int l, int r, int ql, int qr) {
        if (ql > r || qr < l) return 0;
        if (ql <= l && r <= qr) return seg[node];
        int mid = (l + r) / 2;
        return max( query(2 * node, l, mid, ql, qr), query(2 * node + 1, mid + 1, r, ql, qr));
    }
    vector<bool> getResults(vector<vector<int>>& queries) {
        int n = queries.size();
        seg.resize(4 * (MAX + 1), 0);
        set<int> obs;
        obs.insert(0);
        for (auto &q : queries) {
            if (q[0] == 1)
                obs.insert(q[1]);
        }
        vector<int> pos(obs.begin(), obs.end());
        for (int i = 1; i < pos.size(); i++) {
            update(1, 0, MAX, pos[i], pos[i] - pos[i - 1]);
        }
        vector<bool> ans;
        for (int i = n - 1; i >= 0; i--) {
            if (queries[i][0] == 2) {
                int x = queries[i][1];
                int sz = queries[i][2];
                auto it = prev(obs.upper_bound(x));
                int prevObs = *it;
                int best = query(1, 0, MAX, 0, prevObs);
                best = max(best, x - prevObs);
                ans.push_back(best >= sz);
            } else {
                int x = queries[i][1];
                auto it = obs.find(x);
                int leftPos = *prev(it);
                update(1, 0, MAX, x, 0);
                auto rightIt = next(it);
                if (rightIt != obs.end()) {
                    int rightPos = *rightIt;
                    update(1, 0, MAX, rightPos, rightPos - leftPos);
                }
                obs.erase(it);
            }
        }
        reverse(ans.begin(), ans.end());
        return ans;
    }
};